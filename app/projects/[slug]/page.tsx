...
export default async function ProjectDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="page-shell detail-page project-detail-page"><Container>
    ...
    <div className="detail-grid"><main className="detail-main">
      ...
      {sectionNumbers.has("features") && <section className="detail-section" id="features"><div className="section-heading"><span>{sectionNumbers.get("features")}</span><div><h2>Features</h2><p>项目数据中明确记录的能力与边界。</p></div></div><div className="feature-grid">{project.details.map((detail, index) => { const Icon = featureIcons[index % featureIcons.length]; return <article className="feature-card" key={detail}><Icon size={20} /><h3>Feature {String(index + 1).padStart(2, "0")}</h3><p>{detail}</p></article>; })}</div></section>}
