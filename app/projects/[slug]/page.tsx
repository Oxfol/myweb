...
export default async function ProjectDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="page-shell detail-page project-detail-page"><Container>
    ...
    <div className="detail-grid"><main className="detail-main">
      <section className="detail-section" id="overview"><div className="section-heading"><span>01</span><div><h2>Overview</h2><p>当前状态、公开入口与下一步计划。</p></div></div><div className="overview-card"><div><small>Status</small><strong><StatusBadge status={project.status} /></strong></div>{project.repository && <div><small>Repository</small><strong><a href={project.repository} target="_blank" rel="noreferrer">GitHub <ArrowUpRightIcon size={14} /></a></strong></div>}{project.deployedUrl && <div><small>Deployment</small><strong><a href={project.deployedUrl} target="_blank" rel="noreferrer">Online <ArrowUpRightIcon size={14} /></a></strong></div>}{project.currentPhase && <div><small>Current phase</small><strong>{project.currentPhase}</strong></div>}{project.nextStep && <div><small>Next step</small><strong>{project.nextStep}</strong></div>}{project.updatedAt && <div><small>Last updated</small><strong>{project.updatedAt}</strong></div>}</div></section>
      {project.details.length > 0 && <section className="detail-section" id="features"><div className="section-heading"><span>02</span><div><h2>Features</h2><p>项目数据中明确记录的能力与边界。</p></div></div><div className="feature-grid">{project.details.map((detail, index) => { const Icon = featureIcons[index % featureIcons.length]; return <article className="feature-card" key={detail}><Icon size={20} /><h3>Feature {String(index + 1).padStart(2, "0")}</h3><p>{detail}</p></article>; })}</div></section>}
      {project.architecture?.length ? <section className="detail-section" id="architecture"><div className="section-heading"><span>03</span><div><h2>Architecture</h2><p>项目数据中明确记录的系统组成。</p></div></div><div className="architecture-diagram">{project.architecture.map((node, index) => <Fragment key={`${node.label}-${node.value}`}><div className="architecture-node"><small>{node.label}</small><strong>{node.value}</strong></div>{index < project.architecture!.length - 1 && <ArrowRightIcon size={18} />}</Fragment>)}</div></section> : null}
      {project.stack.length > 0 && <section className="detail-section" id="tech-stack"><div className="section-heading"><span>04</span><div><h2>Tech Stack</h2><p>数据源明确声明的技术与服务。</p></div></div><div className="detail-tech-grid">{project.stack.map(item => <TechBadge key={item}>{item}</TechBadge>)}</div></section>}
      {project.timeline?.length ? <section className="detail-section" id="timeline"><div className="section-heading"><span>05</span><div><h2>Development Timeline</h2><p>项目数据中记录的开发节点。</p></div></div><div className="detail-timeline">{project.timeline.map(item => <div key={`${item.date}-${item.title}`}><time>{item.date}</time><div><h3>{item.title}</h3><p>{item.description}</p></div><CheckCircleIcon size={17} /></div>)}</div></section> : null}
      {hasRelatedContent && <section className="detail-section related-section" id="related"><div className="section-heading"><span>06</span><div><h2>Related Content</h2><p>基于已声明技术标签关联的项目与日志。</p></div></div><div className="related-detail-grid">{related.map(item => <Link key={item.slug} href={`/projects/${item.slug}`}><small>PROJECT</small><strong>{item.title}</strong><span>{item.description}</span><ArrowRightIcon size={15} /></Link>)}{relatedLogs.map(item => <Link key={item.slug} href={`/logs/${item.slug}`}><small>LOG · {item.date}</small><strong>{item.title}</strong><span>{item.summary}</span><ArrowRightIcon size={15} /></Link>)}</div></section>}
